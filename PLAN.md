# Планируемые фичи

- [x] Фикс поиска в Spotify
  - [x] Нашёл "Dreamcraft - Original Deep Mix — Following Light"
  - [x] Вместо "Dreamcraft - Original Deep Mix — Following Light, MDeco"
  - [x] Не брать первый подходящий мэтч, а ранжировать кандидатов и выбирать наиболее подходящий

- [x] Баг: Список Queue после перезапуска приложения (точнее после краша), стал пустым, хотя в Spotify в очереди остались треки, добавленные генератором.
  - [x] Найти и исправить баг, если он есть
  - [x] Можно сделать fallback на пересечении очереди из Spotify и recommendation_seeds.yml

- [x] Добавить propagation состояния приложения по настройкам через разные UI: Qt - WebUI - потенциально какой-нибудь ещё.
  - Сейчас при включении чекбокса Replay seeds before recs в WebUI, в Qt интерфейсе не видно изменений.

- Перестроить систему генерации в три отдельных блока: `Seed sources`, `Generators` и `Queue modifiers`
  - Seed sources
    - [x] Отдельный блок UI `Seed sources:`
    - [x] Current playlist / tracks list
    - [x] Random recent Last.fm track
    - [x] Random loved Last.fm track
    - [x] Random recent Last.fm track as seed
    - [x] Random loved Last.fm track as seed
    - [x] Если выбрано несколько source'ов, выбирать один равновероятно
    - [ ] Позже добавить веса и для source'ов
  - Generators
    - [x] Отдельный блок UI `Generators:`
    - [x] Raw seed
    - [x] Similar artist top tracks
    - [x] Similar tracks
    - [x] Same artist
    - [x] У каждого generator'а отдельный weight рядом с чекбоксом
    - [x] Если выбрано несколько generator'ов, выбирать один по весам
  - Queue modifiers
    - [x] Отдельный блок UI `Queue modifiers:`
    - [x] `Replay seed every N generated tracks`
    - [x] Никогда не применять replay к `Raw seed`
    - [x] Убрать старый параметр replay interval в его текущем виде
  - Cross-cutting settings / rules
    - [x] Фильтр на explicit content, в last.fm вроде нет такого, тогда надо смотреть при поиске треков в Spotify
    - [x] Добавить persistent history для already queued/generated треков и подмешивать её в `excluded_track_ids`, чтобы генератор не выдавал повторы
    - [x] Генерировать трек того же артиста по сиду
  - Фильтр треков по определённым словам
  - Фильтр треков по списку тегов

- [x] Вынести все api key и secret в .env, можно использовать gem dotenv для работы с ним.

- [x] Добавить ограничение на количество треков в Queue сверху. Превышая этот параметр планировка генерации не должна срабатывать.

- При выборе плейлиста, нужно чтобы он так же, как сейчас, быстро загружался в список Tracks
  - [x] Все недозагруженные треки из него дозагружаются фоном, с небольшой паузой, а не только при скролле списка Tracks.
  - Надо иметь в виду, что в перспективе нужно будет иметь возможность выбирать сразу несколько плейлистов в качестве источника для seeds. Поэтому загрузка по scroll наверное вообще не вариант будет уже, можно даже выпилить, если не очевиден какой-нибудь компромис.

- Надо добавить возможность работы без Qt фронта вообще. Если будет запускаться приложение, скажем, чисто на VPS.

- Возможность банить сиды/треки
  - Перманентно, на время
  - Иметь доступ к списку банов

- Возможность удалять сиды/треки из источников (Spotify, Last.fm)

- Редизайн Qt приложения
  - Circle bg gradient как в WebUI
  - Collapsable blocks
  - Manual alignment
  - Modals для сложного редактирования (например, track title words blacklist)
  - Убрать лишнее
    - Title/Description,
    - Connect/Disconnect сделать в виде активных/неактивных иконок,
    - Search tracks on Spotify
