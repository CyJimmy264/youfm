...
RSpec.describe YouFM::Services::RecommendationGenerator do
  ...
  it 'skips loose Spotify matches and keeps searching for an exact enough candidate' do
    ...
    allow(spotify_client).to receive(:search_tracks).with('Song Two artist:Exact Artist', limit: 10).and_return([exact_match])
  ...
  it 'moves to the next similar artist after three failed top-track attempts' do
    ...
    allow(spotify_client).to receive(:search_tracks).with('Miss Two artist:First Artist', limit: 10).and_return([loose_match])
    allow(spotify_client).to receive(:search_tracks).with('Miss Three artist:First Artist', limit: 10).and_return([loose_match])
    allow(spotify_client).to receive(:search_tracks).with('Hit Song artist:Second Artist', limit: 10).and_return([exact_match])
    allow_any_instance_of(described_class).to receive(:rand).with(2).and_return(0)
